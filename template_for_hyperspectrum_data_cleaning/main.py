...
if __name__ == '__main__':
    # original dataset
    intensity_folder = r'C:\Users\ap\Desktop\WorkSpace\new_old_grain_classification_based_hyperspectrum\hyperspectrum_intensity'
    hdr_os_dict = {
        "intensity_hdr": r'',
        "intensity_os": r'',
        "dark_hdr": r'',
    ...
    reflc_folder = r'C:\Users\ap\Desktop\WorkSpace\new_old_grain_classification_based_hyperspectrum\hyperspectrum_reflectance'

    # target dataset 
    dataset = r'C:\Users\ap\Desktop\WorkSpace\new_old_grain_classification_based_hyperspectrum\dataset.csv'

    # hyperspectrum data cleaning
    hdr_os_folders_files = get_hdr_os_folders_files(intensity_folder)
    with open(dataset, mode='w', newline='') as file:
        writer = csv.writer(file)
        num = 0
        for sample_name, hdr_file, os_file in hdr_os_folders_files:
            hdr_os_dict["intensity_hdr"] = hdr_file
            hdr_os_dict["intensity_os"] = os_file
            sample_name = sample_name.split('_')[0]

            # create reflectance hyperspectrum's filenames
            base_path = reflc_folder + os.path.sep
            base_name = os.path.splitext(os.path.basename(hdr_os_dict["intensity_hdr"]))[0]
            ...
            reflc_hypspec = reflc_hypspec_calc(hdr_os_dict["intensity_hdr"], hdr_os_dict["intensity_os"], hdr_os_dict["white_hdr"], hdr_os_dict["white_os"], reflc_hdr, reflc_os)
